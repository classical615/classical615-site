...
export async function runBlairScrape({ testLimit } = {}) {
  ...
  // Remembers every Blair ID handled during THIS run. If the RSS feed lists the
  // same event twice, the second copy gets skipped instead of being added again.
  ...
  for (const item of toProcess) {
    try {
      ...
      if (seenThisRun.has(item.sourceId)) {
        summary.skipped.push({ title: item.title, date: item.isoDate, reason: 'duplicate entry in feed - already handled this run' });
        continue;
      }
      seenThisRun.add(item.sourceId);
